...
from word_edit_distance_global_highlighted_average_script import (
    fake_top_words, genuine_top_words)
...
def generate_test_higlighted_dict(output_text, genuine=True):
    highlight_dict = {}
    count_id = 0
    for text_span in output_text.split(" "):
        word = text_span
        if word == "":
            continue
        ...
        highlight = 0
        if genuine:
            for top_word in genuine_top_words:
                if top_word in word.strip():
                    highlight = 1
        else:
            for top_word in fake_top_words:
                if top_word in word.strip():
                    highlight = 1
...
def get_topk_words(output_text, topk_val=0.2, genuine=True):
    word_highlight_dict = generate_test_higlighted_dict(output_text, genuine)
    score_dict = {}
    word_dict = {}
