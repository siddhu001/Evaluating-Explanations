...
parser.add_argument(
    "--user_study_seed", default=1, type=int, help="random seed for user study"
)
...
def predict(input1, explanation_type, test=False):
    ...
    output = user_dict[session.get("name")].model(
    ...
    if test or explanation_type == "global":  # user in test phase
        ...
        if explanation_type == "lime":
            ...
            user_dict[session.get("name")].input_ids = input_ids
            user_dict[session.get("name")].attention_mask = attention_mask
            scores = explainer.get_scores(
                user_dict[session.get("name")].model, user_dict, session
            )
            print(scores)
            scores = scores[0]
            print(scores)
...
def submit():
    ...
    if request.method == "POST":
        ...
        if (
            ...
            if args.global_exp:
                logreg_prob, output_text = logreg_predict(
                    strip_tags(request.form["text"])
                )
